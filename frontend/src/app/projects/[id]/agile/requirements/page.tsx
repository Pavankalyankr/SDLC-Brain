...
export default function RequirementsPage({
  ...
  const isGenerating = genReqs.isPending || isStreamActive;

  const handleGenerate = async () => {
    try {
      const res = await genReqs.mutateAsync({}) as { task_id?: string };
      const invalidate = () => {
        queryClient.invalidateQueries({ queryKey: agileKeys.requirements(projectId) });
      };
      if (res?.task_id) {
  ...
  const handleApproveAll = async () => {
    const drafts = requirements.filter((i) => i.status !== "approved");
    for (const item of drafts) {
      await updateStatus.mutateAsync({ type: "requirements", id: item.id, status: "approved" });
    }
  };
    ...
    <div className="space-y-4">
      <div className="flex justify-end mb-4">
        <ReviewToolbar
          ...
          onApprove={requirements.some((i) => i.status !== "approved") ? handleApproveAll : undefined}
          onExport={async (format) => {
            toast.info(`Preparing ${format.toUpperCase()} export...`);
            try {
              await exportApi.downloadAgileExport(projectId, "requirements", format);
      ...
      {isGenerating && (
        <AIThinking
          message={thinkingMessage || "Generating Requirements..."}
          className="mb-4"
        />
      )}
